...
export interface Config {
  ...
  normalize: NormalizationConfig;
  /**
   * An array of label names that should be excluded from all generated Prometheus metrics.
   * This is useful to prevent cardinality explosions for specific labels (e.g., 'origin').
   */
...
export default {
  default: {
    ...
    excludedLabels: [], // By default, no labels are excluded.
  } as Config,
  validator(config: Config) {
    if (typeof config.collectDefaultMetrics === 'boolean' && config.collectDefaultMetrics) {
      throw Error('Invalid collectDefaultMetrics value. Can only be false or DefaultMetricsCollectorConfiguration');
    ...
    // Validate that excludedLabels is an array of strings.
    if (!Array.isArray(config.excludedLabels) || !config.excludedLabels.every(label => typeof label === 'string')) {
      throw Error('Invalid excludedLabels value. Must be an array of strings.');
    }
  },
};